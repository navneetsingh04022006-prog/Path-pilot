...
const STATS = [
  { label: 'Roadmap progress', value: '0%', icon: TrendingUp, color: 'text-primary dark:text-primary-400', bg: 'bg-primary-light dark:bg-primary-950/60' },
  { label: 'Skills completed', value: '0', icon: CheckCircle, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/60' },
  { label: 'Resources saved', value: '0', icon: BookOpen, color: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-950/60' },
  { label: 'Active goal', value: 'None', icon: Target, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/60' },
];
...
function DashboardPage() {
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      {/* Welcome */}
      <div>
        <h1 className="text-display text-slate-900 dark:text-white">Welcome back, Navneet</h1>
        ...
        {STATS.map(({ label, value, icon: Icon, color, bg }) => (
          <Card key={label} className="flex items-center gap-4 p-5">
            ...
            <div>
              ...
              <p className="text-heading text-slate-900 dark:text-slate-100">{value}</p>
            </div>
          </Card>
        ))}
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="rounded-input border border-dashed border-border dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 px-6 py-10 text-center">
              <Target className="mx-auto mb-3 h-10 w-10 text-slate-300 dark:text-slate-600" aria-hidden="true" />
      ...
      <div>
        <h2 className="mb-4 text-heading text-slate-900 dark:text-slate-100">Today&apos;s goal</h2>
        <Card>
          <CardContent>
